...
SELECT c.marca, c.modelo, CAST(SUM(v.distanciaPercorrida) AS decimal(7,2)) AS Soma_Distancia
FROM empresa e INNER JOIN carro c 
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id 
GROUP BY c.marca, c.modelo
ORDER BY Soma_Distancia
...
SELECT e.nome, CAST(AVG(v.distanciaPercorrida) AS decimal(7,2)) AS mediaDist
FROM empresa e INNER JOIN carro c 
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id 
GROUP BY e.nome
...
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id 
WHERE v.distanciaPercorrida = (
	SELECT MAX(v.distanciaPercorrida) FROM
	viagem v
)
...
SELECT e.nome, COUNT(c.idEmpresa) AS Quantidade_Carros
FROM empresa e INNER JOIN carro c 
ON e.id = c.idEmpresa
GROUP BY e.nome
HAVING COUNT(c.idEmpresa) >= 3
ORDER BY Quantidade_Carros DESC

--6) Consultar empresas que não tem carros cadastrados.
SELECT e.nome 
...
SELECT e.nome, c.marca, c.modelo, COUNT(v.idCarro) AS Quantidade_Viagens
FROM empresa e INNER JOIN carro c
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id
GROUP BY c.marca, c.modelo, e.nome
ORDER BY e.nome ASC, Quantidade_Viagens DESC
...
SELECT e.nome, c.marca, c.modelo, v.distanciaPercorrida, 
	CASE WHEN (v.distanciaPercorrida < 1000) THEN
		CAST('R$' + CAST((v.distanciaPercorrida * 10) AS VARCHAR(10)) AS VARCHAR(10))
		ELSE 
		CAST('R$' + CAST((v.distanciaPercorrida * 15) AS VARCHAR(10)) AS VARCHAR(10))
END AS Valor_Total
FROM empresa e INNER JOIN carro c
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id
...
SELECT e.nome, CAST(AVG(v.distanciaPercorrida) AS DECIMAL(7,2)) AS media
FROM empresa e INNER JOIN carro c
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id
GROUP BY e.nome
...
SELECT e.nome, c.marca, c.modelo, CONVERT(CHAR(10), v.data, 103) as Data_Ultima_viagem
FROM empresa e INNER JOIN carro c
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id
GROUP BY e.nome, c.marca, c.modelo, v.data
HAVING v.data = (
	SELECT MAX(v.data)
	FROM viagem v
)

--12) Considerando que hoje é 01/01/2023, apresentar a marca e o modelo do carro, além do nome da empresa e a quantidade de dias da viagem, dos carros que 
--tiveram viagens nos últimos 3 meses. Ordenar (todos ascendentes) por quantidade de dias, marca, modelo e nome da empresa.
SELECT e.nome, c.marca, c.modelo, DATEDIFF(DAY, v.data,'01-01-2023') AS dias_viagem
FROM empresa e INNER JOIN carro c
ON e.id = c.idEmpresa INNER JOIN viagem v
ON v.idCarro = c.id
WHERE v.data BETWEEN DATEADD(MONTH, -3, '2023-01-01') AND '2023-01-01'
ORDER BY dias_viagem, c.marca, c.modelo, e.nome
